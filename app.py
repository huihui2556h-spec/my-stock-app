...
# --- 分頁：盤中即時 ---
    ...
    stock_id = st.text_input("輸入代碼 (例如: 8112):", key="rt_key")
    if stock_id:
        with st.spinner('讀取即時數據...'):
            ...
            if df_rt is not None:
                ...
                st.warning("⚠️ 抓不到盤中數據。可能原因：1. 非開盤時間 2. 代碼錯誤。")

# --- 分頁：深度預估 ---
elif st.session_state.mode == "forecast":
    if st.sidebar.button("⬅️ 返回首頁"): navigate_to("home")
    st.title("📊 深度預估分析")
    stock_id = st.text_input("輸入代碼 (例如: 2330):", key="fc_key")
    if stock_id:
        with st.spinner('分析多因子因子...'):
            ...
            if df is not None:
                ...
                v1, v2 = st.columns(2)
                v1.metric("收盤價", f"{curr_c:.2f}")
                v2.metric("預估開盤", f"{est_open:.2f}", f"{est_open-curr_c:+.2f}")
                ...
                st.pyplot(fig)
                st.info("📘 圖表說明：顯示收盤價走勢與預期波動。")
            else:
                st.error("❌ 找不到此代碼，請確認代碼是否正確。")
